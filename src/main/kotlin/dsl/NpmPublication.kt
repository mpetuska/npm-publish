...
/**
   * Flag to bundle kotlin-only dependencies. Defaults to true and only works for autogenerated
   * kotlin publications.
   */
  var bundleKotlinDependencies: Boolean by project.propertyDelegate(propGroup) {
    it.notFalse()
  } or extension.fallbackDelegate(NpmPublishExtension::bundleKotlinDependencies)
...
/** Flag to add bundled dependencies to npm-shrinkwrap.json. Defaults to true. */
  var shrinkwrapBundledDependencies: Boolean by project.propertyDelegate(propGroup) {
    it.notFalse()
  } or extension.fallbackDelegate(NpmPublishExtension::shrinkwrapBundledDependencies)
...
/**
   ...
   * 'NODE_HOME' env variable.
   */
  var nodeJsDir by project.propertyDelegate(propGroup, System.getenv("NODE_HOME")?.let(::File)) {
    File(it)
  }
...
/**
  ...
  internal var packageJsonSpecs = mutableListOf<Builder<PackageJson>>()
...
/**
   * If [packageJsonFile] is not set and this property is set, ignore any further package.json
   * generation configs produced by DSLs
   */
  var packageJson by project.gradleNullableProperty<(Builder<PackageJson>)>()

/** DSL builder to configure generated package.json file. */
  @PackageJsonDsl
  fun packageJson(config: Builder<PackageJson>) {
...
/** DSL builder to configure NPM dependencies for this publication. */
  fun dependencies(config: Builder<MutableList<NpmDependency>>) = npmDependencies.config()
